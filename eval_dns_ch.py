...
def load_enhancer_dccrn(ckpt_path: str, device: torch.device):
    dccrn = DCCRN(
        rnn_units=256,
        masking_mode="E",
        use_clstm=True,
    ...
    for k, v in sd.items():
        if k.startswith("model."):
            dccrn_sd[k[len("model."):]] = v

    if len(dccrn_sd) == 0:
        for k, v in sd.items():
            if k.startswith("model.model."):
                dccrn_sd[k[len("model.model."):]] = v

    if len(dccrn_sd) == 0:
        raise RuntimeError(f"Couldn't find DCCRN weights in: {ckpt_path}")

    dccrn.load_state_dict(dccrn_sd, strict=False)
    dccrn.to(device).eval()
    return dccrn
...
def enhance_chunked_ola(
    ...
    for start in range(0, T, H):
        ...
        with maybe_autocast(use_autocast, device):
            y = _get_wav_from_enh_out(enhancer(x, emb=emb))
...
def enhance_full_or_chunked(
    ...
    try:
        with maybe_autocast(use_autocast, device):
            enh_out = enhancer(noisy_bt, emb=emb)
...
def main():
    ...
    ap.add_argument("--out_dir", type=str, default="./dns_p835_out")
    ...
    enhancer = load_enhancer_dccrn(args.enh_ckpt, device)
